...
def test_make_phantom_mask():
    # Don't make expmask too small; make_phantom_mask, at least with the
    ...
    dnii = nib.nifti1.Nifti1Image(data, aff)
    mask = dpe.make_phantom_mask(dnii, bvals, 0.0)
    softener = np.ones((3, 3, 3))
    softener /= softener.sum()
    s0 = convolve(s0, softener, mode='constant')
    expmask = np.zeros_like(phantom)
    expmask[s0 > 5] = 1
